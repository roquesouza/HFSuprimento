...
public final class CotacaoProdutoFornecedorDAO implements Serializable {
	...
	public CotacaoFornecedorModel obter(String hash) {

		TSDataBaseBrokerIf broker = TSDataBaseBrokerFactory.getDataBaseBrokerIf();

		broker.setSQL("SELECT DISTINCT CF.ID, CF.COTACAO_ID, F.ID, F.NOME_FANTASIA, CF.CONDICAO_PAGAMENTO, CF.FRETE, CF.PRAZO_ENTREGA, CF.DATA_ATUALIZACAO FROM FORNECEDOR F, COTACAO_FORNECEDOR CF, COTACAO C WHERE F.ID = CF.FORNECEDOR_ID AND C.ID = CF.COTACAO_ID AND C.STATUS_COTACAO_ID = 2 AND C.DATA_VALIDADE >= CURRENT_DATE AND MD5(CF.ID::TEXT) = ?", hash);

		CotacaoFornecedorModel model = (CotacaoFornecedorModel) broker.getObjectBean(CotacaoFornecedorModel.class, "id", "cotacaoModel.id", "fornecedorModel.id", "fornecedorModel.nomeFantasia", "condicaoPagamento", "frete", "prazoEntrega", "dataAtualizacao");
	...
	public CotacaoFornecedorModel alterar(CotacaoFornecedorModel model) throws TSApplicationException {
		
		TSDataBaseBrokerIf broker = TSDataBaseBrokerFactory.getDataBaseBrokerIf();
		
		broker.setSQL("UPDATE COTACAO_FORNECEDOR SET CONDICAO_PAGAMENTO = ?, FRETE = ?, PRAZO_ENTREGA = ?, DATA_ATUALIZACAO = ?, USUARIO_ATUALIZACAO_ID = NULL WHERE ID = ?", model.getCondicaoPagamento(), model.getFrete(), model.getPrazoEntrega(), new Timestamp(model.getDataAtualizacao().getTime()), model.getId());
